-- Admins may add photographs without the member daily upload cap.
-- Status still enters as pending; the app publishes after the files are stored.
...
begin
  ...
  if not public.is_verified_member() and not public.is_admin() then
